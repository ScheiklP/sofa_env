...
def camera_control(controller: XboxController, env: RealtimeWrapper) -> None:
    lx, ly, rx, ry, lt, rt = controller.read()
    env.camera.set_pose(env.camera.get_pose() + np.array([0, 0, -ry] + [0, 0, 0, 0], dtype=np.float32))
    env.camera.set_pose(env.camera.get_pose() + np.array([-rx, 0, 0] + [0, 0, 0, 0], dtype=np.float32))

    env.camera.set_pose(env.camera.get_pose() + np.array([0, -rt, 0] + [0, 0, 0, 0], dtype=np.float32))
    env.camera.set_pose(env.camera.get_pose() + np.array([0, lt, 0] + [0, 0, 0, 0], dtype=np.float32))

    old_pose = env.camera.get_pose()
    orientation_delta = rotation_matrix_to_quaternion(euler_to_rotation_matrix(np.array([-ly, 0, 0])))
    ...
    old_pose = env.camera.get_pose()
    orientation_delta = rotation_matrix_to_quaternion(euler_to_rotation_matrix(np.array([0, -lx, 0])))
    new_pose = np.zeros_like(old_pose)
    new_pose[:3] = old_pose[:3]
    new_pose[3:] = multiply_quaternions(orientation_delta, old_pose[3:])
    env.camera.set_pose(new_pose)
    ...
    old_pose = env.camera.get_pose()
    orientation_delta = rotation_matrix_to_quaternion(euler_to_rotation_matrix(np.array([0, 0, controller.right_bumper])))
    new_pose = np.zeros_like(old_pose)
    new_pose[:3] = old_pose[:3]
    new_pose[3:] = multiply_quaternions(orientation_delta, old_pose[3:])
    env.camera.set_pose(new_pose)
    ...
    old_pose = env.camera.get_pose()
    orientation_delta = rotation_matrix_to_quaternion(euler_to_rotation_matrix(np.array([0, 0, -controller.left_bumper])))
    new_pose = np.zeros_like(old_pose)
    new_pose[:3] = old_pose[:3]
    new_pose[3:] = multiply_quaternions(orientation_delta, old_pose[3:])
    env.camera.set_pose(new_pose)
    ...
    if controller.a:
        print(repr(env.camera.get_pose()))
...
if __name__ == "__main__":
    ...
    env = TissueDissectionEnv(
        ...
        frame_skip=int(1.0 / (30.0 * 0.01)),
        time_step=0.01,
        settle_steps=50,
        create_scene_kwargs={"show_border_point": False},
    )
