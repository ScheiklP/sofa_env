...
class EpisodicEnvWrapper(gym.Wrapper):
    ...
    Args:
        ...
        stop_on_done (bool): Whether to stop the episode when the environment returns ``done=True``.
    ...
    def step(self, actions):
        ...
        Returns:
            If ``return_buffers`` is ``False``:
                ...
                done (bool): Whether the episode is done in the last step.
            ...
            If ``return_buffers`` is ``True``:
                ...
                done_buffer (np.ndarray): The done flags of the episode.
        ...
        if self.return_buffers:
            ...
            done_buffer = np.zeros(self.episode_length, dtype=np.bool8)
            info_list = []
        ...
        if self.return_buffers:
            return observation_buffer, reward_buffer, done_buffer, info_list, reward
        else:
